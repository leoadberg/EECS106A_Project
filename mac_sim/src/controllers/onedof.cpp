...
vector<mjtNum> goalPos;
vector<mjtNum> integral;
vector<mjtNum> ctrl;
bool firstTime = true;

void onedofcontroller(const mjModel* m, mjData* d)
{
    if (firstTime) {
      firstTime = false;
...
//    ctrl = 3.5*(-d->qvel[0]-10.0*d->qpos[0]);
    ...
    //     position_history = d->sensordata[0];
        ...
        for (int i = 0; i < NUM_CONTROLS; i++) {
          mjtNum err = goalPos[i] - d->qpos[i];
          integral[i] += err * (d->time - previous_time);

          ctrl[i] = (KP * err) + (KI * integral[i]) - (KD * d->qvel[i]);

          cout << "Ctrl " << i << " = " << ctrl[i] << endl;
        }
